...
\chapter{Breve API de los Contenedores}
...
\begin{apientry}{type * name\_scope\_init(name\_scope * vector, int size)}{Constructor}
    Inicializa el vector con al menos \mono{size} espacios. El tamaño en memoria se modifica dinámicamente según se añadan o eliminen elementos.     
\end{apientry}
...
\begin{apientry}{type * search(* self, int (*comp)(type value, type elemento) )}{Método}
    Busca si \mono{value} se encuentra almacenado. Necesita la unción \mono{comp(type value, type)} para comparar entre elementos, la cual retorna no cero si los elementos comparados son iguales.  
\end{apientry}
